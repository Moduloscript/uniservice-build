...
export function useOptimizedFileUpload(options: FileUploadOptions = {}) {
  ...
  const getSignedUrlMutation = useMutation({
    mutationFn: async (params: { bucket: string; path: string }): Promise<UploadResponse> => {
  ...
  const uploadToSignedUrlMutation = useMutation({
    mutationFn: async (params: { signedUrl: string; file: File; path: string }): Promise<string> => {
  ...
  const uploadFile = useCallback(async (params: UploadFileParams): Promise<string | undefined> => {
    ...
    try {
      ...
      const path = customPath || `${options.userId ? `${options.userId}-` : ""}${uuid()}.${ext}`;
      ...
      // Upload file
      const uploadedPath = await uploadToSignedUrlMutation.mutateAsync({
        signedUrl,
        file,
        path,
      });
  ...
  const isUploading = getSignedUrlMutation.isPending || uploadToSignedUrlMutation.isPending;
  const error = getSignedUrlMutation.error?.message || uploadToSignedUrlMutation.error?.message;

...
export function ServiceFeatures({ serviceId, className = "" }: ServiceFeaturesProps) {
    ...
    if (isLoading) {
        return (
            <Card className={cn("transition-all duration-200", className)}>
                ...
                <CardContent>
                    <div className="space-y-3" role="status" aria-label="Loading service features">
                        {[...Array(4)].map((_, i) => (
            ...
            <Card className={cn("bg-muted/30", className)}>
                ...
                <CardContent>
                    <p className="text-muted-foreground text-sm">
                        No features have been added to this service yet. The provider may add custom features soon.
                    </p>
                </CardContent>
            </Card>
        );
    ...
    return (
        <Card className={cn("transition-all duration-200 hover:shadow-md", className)}>
            ...
            <CardContent>
                <div className="space-y-4" role="list" aria-label="Service features">
                ...
                {features.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-muted">
                        <p className="text-xs text-muted-foreground text-center">
                            {features.length} feature{features.length !== 1 ? 's' : ''} included with this service
                        </p>
                    </div>
                )}
            </CardContent>

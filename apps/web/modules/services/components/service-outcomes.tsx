...
export function ServiceOutcomes({ serviceId, className = "" }: ServiceOutcomesProps) {
    const {
        data: outcomes = [],
        isLoading,
        isError,
            ...
            <Card className={cn("transition-all duration-200", className)}>
                <CardHeader className="pb-4">
                    <CardTitle className="flex items-center gap-2">
                        ...
                        <span className="text-lg">Loading learning outcomes...</span>
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <div className="space-y-3" role="status" aria-label="Loading service learning outcomes">
                        {[...Array(4)].map((_, i) => (
            ...
            <Card className={cn("border-destructive/20 bg-destructive/5", className)}>
                <CardHeader className="pb-4">
                    <CardTitle className="flex items-center gap-2 text-destructive">
                        ...
                        <span className="text-lg">Error Loading Learning Outcomes</span>
                    </CardTitle>
                </CardHeader>
                <CardContent>
                    <p className="text-destructive/80 text-sm" role="alert">
                        {formatErrorMessage(error, "Failed to load service learning outcomes")}
                    </p>
                </CardContent>
            </Card>
        );
            ...
            <Card className={cn("bg-muted/30", className)}>
                ...
                <CardContent>
                    <p className="text-muted-foreground text-sm">
                        No learning outcomes have been defined for this service yet. The provider may add specific learning goals soon.
                    </p>
                </CardContent>
            </Card>
        );
    ...
    return (
        <Card className={cn("transition-all duration-200 hover:shadow-md", className)}>
            ...
            <CardContent>
                <div className="space-y-4" role="list" aria-label="Service learning outcomes">
                ...
                {outcomes.length > 0 && (
                    <div className="mt-4 pt-4 border-t border-muted">
                        <p className="text-xs text-muted-foreground text-center">
                            {outcomes.length} learning outcome{outcomes.length !== 1 ? 's' : ''} defined for this service
                        </p>
                    </div>
                )}
            </CardContent>
...
export function ServiceOutcomesServer({ outcomes, className = "" }: ServiceOutcomesServerProps) {
    if (outcomes.length === 0) {
        return (
            <Card className={cn("bg-muted/30", className)}>
                <CardHeader className="pb-4">

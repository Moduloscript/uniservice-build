...
export const providerAvailabilityApi = {
  ...
  async getAvailability(
    ...
    const response = await fetch(`/api/providers/${providerId}/availability?${new URLSearchParams(query)}`);
  ...
  ): Promise<{ success: boolean; data: ProviderAvailabilitySlot; message: string }> {
  ...
  async updateSlot(
    ...
    const response = await fetch(`/api/providers/${providerId}/availability/${availabilityId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Failed to update availability slot');
    }
  ...
  async createBulkSlots(
    ...
    const response = await fetch(`/api/providers/${providerId}/availability/bulk`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Failed to create bulk availability slots');
    }
  ...
  async createRecurringSchedule(
    ...
    for (let week = 0; week < weeksAhead; week++) {
      ...
      targetDate.setDate(startDate.getDate() + (week * 7) + (schedule.dayOfWeek - startDate.getDay()));
...
export const providerAvailabilityQueryKeys = {
  ...
  byProvider: (providerId: string) => ['provider-availability', providerId] as const,
  availability: (
    providerId: string, 
    params?: { startDate?: string; endDate?: string; serviceId?: string }

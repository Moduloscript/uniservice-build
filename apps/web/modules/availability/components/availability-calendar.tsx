...
import type { AvailabilityCalendarProps, AvailabilityTimeSlot, ProviderAvailability } from "../types";
...
export function AvailabilityCalendar({
  ...
  const timeSlots = useMemo(() => {
    return availabilityData.map((slot: ProviderAvailability): AvailabilityTimeSlot => {
      const date = slot.date;
      const startTime = slot.startTime;
      const endTime = slot.endTime;
      
      // Format display time (e.g., "2:00 PM - 3:00 PM")
      const startTimeFormatted = format(parseISO(`1970-01-01T${startTime}`), "h:mm a");
      const endTimeFormatted = format(parseISO(`1970-01-01T${endTime}`), "h:mm a");
  ...
  const slotsForSelectedDate = useMemo(() => {
    ...
    return timeSlots.filter((slot) => {
      ...
      return slotDate === selectedDateStr && slot.isAvailable && (showBookedSlots || !slot.isBooked);
    });
  ...
  useEffect(() => {
    if (availableDates.length > 0 && !availableDates.some(date => isSameDay(date, selectedDate))) {
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-center py-8">
            ...
            <h3 className="font-semibold text-lg mb-2">No Availability Yet</h3>
            <p className="text-muted-foreground">
              The provider hasn't set up their availability schedule yet.
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-7 gap-2">
            {next14Days.map(({ date, hasAvailability, isToday }) => (
              <Button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {slotsForSelectedDate.map((slot) => (
                <Button
                  ...
                  size="lg"
                  disabled={readonly || slot.isBooked || slot.availableSpots === 0}
        ...
        {!readonly && slotsForSelectedDate.length > 0 && (
          <div className="bg-muted/50 p-4 rounded-lg">
            <h5 className="font-medium mb-2">Booking Instructions</h5>
            <ul className="text-sm text-muted-foreground space-y-1">
              <li>• Select your preferred time slot above</li>
              <li>• Click "Book Now" to proceed with your booking</li>
              <li>• You'll receive a confirmation email once booked</li>
            </ul>
          </div>
        )}
      </CardContent>

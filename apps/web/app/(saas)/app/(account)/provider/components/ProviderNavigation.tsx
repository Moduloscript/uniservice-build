  ...
  const generateBreadcrumbs = () => {
    ...
    if (segments.includes("provider")) {
      ...
      if (segments.includes("services")) {
        breadcrumbs.push({ label: "Services", href: "/app/provider/services" });
        
        if (segments.includes("new")) {
          breadcrumbs.push({ label: "New Service", href: "/app/provider/services/new" });
            ...
            <div className="flex items-center space-x-4">
              ...
              {stats && (
                <div className="hidden sm:flex items-center space-x-2 text-sm text-muted-foreground">
                  <span>{stats.services?.total || 0} Services</span>
                  <Separator orientation="vertical" className="h-4" />
                  <span>{stats.bookings?.total || 0} Bookings</span>
                </div>
              )}

...
import { ServiceOutcome } from "../../../../../../../modules/services/types/service-outcome";

export default function ProviderOutcomesManagementPage({ params }: { params: { serviceId: string } }) {
  const {
    outcomes,
    isLoading, 
    error, 
  ...
  const [editingOutcome, setEditingOutcome] = useState<ServiceOutcome | null>(null);
  
  const sensors = useSensors(
    useSensor(PointerSensor),
  );

  const handleAddOutcome = (data: Omit<ServiceOutcome, 'id' | 'serviceId'>) => {
    addOutcome(data);
  };

  const handleDeleteOutcome = (outcomeId: string) => {
    if (confirm('Are you sure you want to delete this outcome? This action cannot be undone.')) {
  ...
  return (
    <main className="max-w-4xl mx-auto py-6 px-4 space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-muted-foreground">
            Here are the learning outcomes available for your service. Add new outcomes, edit or remove existing ones, and reorder them to suit your preference.
          </p>
        </CardContent>
      </Card>
      ...
      <OutcomeManagementForm 
        ...
        onSubmit={editingOutcome ? data => updateOutcome({ outcomeId: editingOutcome.id, data }) : handleAddOutcome}
      ...
      {isLoading ? (
        ...
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleSortEnd}>
          <SortableContext items={outcomes.map(outcome => outcome.id)} strategy={verticalListSortingStrategy}>
              ...
              <SortableItem key={outcome.id} id={outcome.id}>
                <Card className="mb-2">
                  <CardHeader className="flex items-center justify-between px-4 py-2">
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="ghost" size="sm" onClick={() => setEditingOutcome(outcome)}>Edit</Button>
                      <Button variant="ghost" size="sm" onClick={() => handleDeleteOutcome(outcome.id)}>

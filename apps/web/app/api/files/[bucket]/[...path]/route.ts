...
export async function GET(request: NextRequest, context: RouteContext) {
	try {
		...
		console.log(
			`File access attempt: bucket=${bucket}, path=${filePath}, user=${session.user.email}, role=${session.user.role}`,
		);
		...
		if (
			...
			console.error(
				`Forbidden access attempt by non-admin user: ${session.user.email}, role: ${session.user.role}`,
			);
			return new Response(
				"Forbidden - Admin access required for document verification",
				{ status: 403 },
			);
		}
		...
		if (!fileResponse.ok) {
			console.error(
				`Failed to fetch file from storage: ${fileResponse.status} ${fileResponse.statusText}`,
			);
			return new Response("File not found", { status: 404 });
		}

...
app.post(
  ...
  async (c) => {
    ...
    const { serviceId, date, startTime, endTime, maxBookings, notes } = c.req.valid("json");

    // Check if user is the provider or admin
    if (user.id !== providerId && user.userType !== "ADMIN") {
      return c.json(
        { success: false, error: "Unauthorized to manage this provider's availability" },
        403
      );
    }
    ...
    try {
      ...
      if (existingSlot) {
        return c.json(
          { success: false, error: "Overlapping time slot already exists" },
          400
        );
      }
...
app.post(
  ...
  async (c) => {
    ...
    try {
      ...
      return c.json(
        { success: false, error: "Failed to create availability slots" },
        500
      );
    }
  }

...
export const servicesRouter = new Hono()
	...
	.get("/", async (c) => {
		...
		const servicesWithRatings = includeRatings
	...
	.get(":id", async (c) => {
		...
		if (!z.string().uuid().safeParse(id).success) {
			console.log(
				`[Services API] Invalid service ID format for ID: ${id}`,
			);
		...
		try {
			...
			console.log(
				`[Services API] Successfully found service: ${service.name}`,
			);
			...
			const serviceWithRatings = {
				...
				ratingStats: {
					...
					averageRating: Number(averageRating.toFixed(1)),
					lastReviewDate:
						reviews.length > 0 ? reviews[0].createdAt : null,
				},
			};
	...
	.put(
		":id",
		authMiddleware,
		validator("json", createServiceSchema.partial()),
		async (c) => {
			const id = c.req.param("id");
			if (!z.string().uuid().safeParse(id).success) {

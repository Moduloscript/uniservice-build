...
const serviceIdSchema = z.object({
	serviceId: z
		...
		.min(1)
		.max(50)
		.regex(/^[a-zA-Z0-9_-]+$/, "Invalid service ID format"),
});
...
const outcomeIdSchema = z.object({
	outcomeId: z
		...
		.min(1)
		.max(50)
		.regex(/^[a-zA-Z0-9_-]+$/, "Invalid outcome ID format"),
});
	...
	.post(
		...
		async (c) => {
			...
			if (service.providerId !== user.id) {
				return c.json(
					{
						error: "Unauthorized - You can only manage outcomes for your own services",
					},
					403,
				);
			}
